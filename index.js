...
const CARRITO_DE_COMPRAS = {
    ...
    metodos: {
        agregar: (id, cantidad) => {
            ...
            actualizarElementosStorage()
            obtenerElementosStorage()
        },
        borrar: (id, cantidad) => {
            const ELEMENTO_CARRITO = CARRITO_DE_COMPRAS.metodos.obtener(id);
...
function actualizarElementosStorage() {
    let elementosJSON = JSON.stringify(CARRITO_DE_COMPRAS.elementos)
    localStorage.setItem("elementos", elementosJSON)
}

function obtenerElementosStorage() {
    let elementosJSON = localStorage.getItem("elementos")
    if(elementosJSON) {
        CARRITO_DE_COMPRAS.elementos = JSON.parse(elementosJSON)
        renderShoppingCart()
    }
}
